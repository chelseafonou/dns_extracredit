...
def dns_query(type, name, server):
    ...
    server_address = (server, 53)  # DNS operates on port 53
    ...
    if type == 'A':
        qtype = 1  # A record type value
    elif type == 'AAAA':
        qtype = 28  # AAAA record type value
    else:
        raise ValueError('Invalid type')

    qclass = 1  # IN class value
    ...
    response_header = data[:12]  # DNS response header is 12 bytes
    ID, FLAGS, QDCOUNT, ANCOUNT, NSCOUNT, ARCOUNT = struct.unpack('!HHHHHH', response_header)

    # Parse the response question section
